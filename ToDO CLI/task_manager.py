...
def create_task(title=None, priority=None, status=None):
    ...
    if title is None:
        while True:
            title = helper.sanitize_string(input("Enter Task Title: "))

            if not valid_title(title):
                logger.warning("Invalid task title entered.")
                print("Please enter a valid task title.")
                continue

            if helper.task_exists(tasks, title):
                logger.warning(f"Duplicate task title: {title}")
                print("Task already exists. Please enter another title.")
                continue
        ...
        if not valid_title(title):
            logger.warning("Invalid task title from command line.")
            print("Please enter a valid task title.")
            return
    ...
    if priority is None:
        while True:
            priority = helper.normalize_priority(input("Enter Priority (High/Medium/Low): "))

            if not valid_priority(priority):
                logger.warning("Invalid priority entered.")
        ...
        if not valid_priority(priority):
            logger.warning("Invalid priority from command line.")
            print("Invalid Priority.")
            return

    if status is None:
        while True:
            status = helper.normalize_status(
                input("Enter Status (Pending/In Progress/Completed): ")
            )

            if not valid_status(status):
                logger.warning("Invalid status entered.")
        ...
        if not valid_status(status):
            logger.warning("Invalid status from command line.")
...
def update_task(task_id=None,title=None,priority=None,status=None):
    ...
    if search_by == "Search by ID":
        ...
        title = helper.sanitize_string(questionary.text("Enter Task Title:").ask())

        task = helper.find_task_by_title(tasks, title)
    ...
    if field == "Title":
        ...
        while True:
            new_title = helper.sanitize_string(questionary.text("Enter New Title:").ask())

            if not valid_title(new_title):
                print("Please enter a valid title.")
                continue

            if helper.task_exists(tasks, new_title):
                print("Task already exists.")
                continue
        ...
        while True:
            new_priority = helper.normalize_priority(questionary.text("Enter New Priority:").ask())
        ...
        while True:
            new_status = helper.normalize_status(questionary.text("Enter New Status:").ask())
            if not valid_status(new_status):
                print("Invalid Status.(In Progress/Pending/Completed)")
                continue
            task["status"] = new_status
            # If task is marked as completed, ask whether to archive it
            if task["status"] == "Completed":

                archive_now = questionary.confirm("This task has been marked as Completed.\n""Would you like to archive it now?").ask()
                if archive_now:
        ...
        while True:
            new_title = helper.sanitize_string(questionary.text(f"Title ({task['title']}):").ask())
        ...
        while True:
            new_priority = helper.normalize_priority(questionary.text(f"Priority ({task['priority']}):").ask())
        ...
        while True:
            new_status = helper.normalize_status(questionary.text(f"Status ({task['status']}):").ask())
            if not valid_status(new_status):
                print("Invalid Status.")
                continue
            task["status"] = new_status
    ...
    confirm = questionary.confirm("Save these changes?").ask()

    if confirm:
        save_tasks(tasks)
        print("\nTask updated successfully!")
        logger.info(f"Task Updated | ID={task['id']}")
        logger.info(f"Update cancelled | ID={task['id']}")
