...
def create_task(title=None, priority=None, status=None):
    ...
    while True:
        if priority is None:
            priority = helper.normalize_priority(input("Enter Priority: "))
        else:
    ...
    while True:
        if status is None:
            status = helper.normalize_status(input("Enter Status: "))
        else:
    ...
    confirm = questionary.confirm("Create this task?").ask()
    if not confirm:
    ...
    logger.warning("Task creation failed due to invalid title.")
